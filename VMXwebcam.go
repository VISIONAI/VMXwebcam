...
func main() {
	...
	fmt.Printf("%s\n",addy)
        ...
        cmd2 := exec.Command(cur_dir+"/VMXwebcam2",os.Args[1])
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		my_channel <- 1
		//log.Printf(r.RemoteAddr+" "+r.UserAgent()+"Locked\n")
		result_file := "/tmp/VMXwebcam."+strconv.Itoa(counter)+".jpg"
		...
		_ = os.Remove(result_file)
                <-my_channel                           
		//log.Printf(r.RemoteAddr+" "+r.UserAgent()+"Unlocked\n")
	})
